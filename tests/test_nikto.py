...
def test_nikto_json_detection(tmp_path: Path):
    ...
    assert parser.can_parse(path, content) is True
...
def test_nikto_json_maps_to_findings(tmp_path: Path):
    ...
    assert all(finding.source_tools == ["nikto"] for finding in findings)
    assert findings[0].asset == "https://app.example.test"
...
def test_nikto_text_maps_findings_and_evidence(tmp_path: Path):
    ...
    assert all(finding.asset == "app.example.test" for finding in findings)
    assert all(finding.source_tools == ["nikto"] for finding in findings)
    assert any(
...
def test_nikto_text_extracts_cve(tmp_path: Path):
    ...
    assert any("CVE-2024-1234" in finding.cve_ids for finding in findings)
